...
export default class Core {
    db(name, version, onUpgradeCallback = () => {}, onBlockedCallback = () => {}){
        try{
            let openPromise;
            ...
            if (version) {
                ...
                openPromise = PromiseWrapper.IDBOpenDBRequest(ENV.indexedDB.open(name, version));
            } else {
                Log.info("db opening", name);
                openPromise = PromiseWrapper.IDBOpenDBRequest(ENV.indexedDB.open(name));
            }
        }
        catch(ex) {
