...
export default class AsyncOperations {
    static db(name, version){
        try{
            ...
            if (version) {
                Log.info("db opening", name, version);
                req = ENV.indexedDB.open(name, version);
            } else {
                Log.info("db opening", name);
                req = ENV.indexedDB.open(name);
            }
        }
        catch(ex) {
...
var async = {
    ...
    createIndex: function (pw, objectStore, propertyName, indexOptions) {
        ...
        try {
            ...
            var indexName = indexOptions ? indexOptions.indexName : undefined;
            if (!indexName) {
    ...
    deleteIndex: function (pw, objectStore, index) {
        ...
        try {
            ...
            linq2indexedDB.logging.log(linq2indexedDB.logging.severity.information, "deleteIndex completed", objectStore.transaction, index, objectStore);
            pw.complete(this, [objectStore.transaction, index, objectStore]);
        } catch (ex) {
        ...
        function insertBatch(i) {
            try {
                ...
                req.then(function (args /*result, e*/) {
                    ...
                    linq2indexedDB.logging.log(linq2indexedDB.logging.severity.information, "Insert completed", data, result, transaction, e);
                    pw.progress(context, [data, result, transaction, e]);

                    if (itemsToProcess === 0) {
                        core.dbDataChanged.fire({
                            type: dataEvents.dataInserted,
                            data: dataresults,
                            objectStore: objectStore
                        });
                        linq2indexedDB.logging.log(linq2indexedDB.logging.severity.information, "Insert Batch completed", dataresults, transaction);
                        pw.complete(context, [dataresults, transaction]);
                    }
                }, function (args /*error, e*/) {
                    var err = wrapError(args[1], "insert");
    ...
    deleteDb: function (pw, name) {
        try {
            if (typeof (indexedDB.deleteDatabase) !== "undefined") {

                handlers.IDBBlockedRequest(indexedDB.deleteDatabase(name)).then(function (args /*result, e*/) {
                    var result = args[0];
                    var e = args[1];
